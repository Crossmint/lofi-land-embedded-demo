...
function CardPurchase({ count, setShowCongrats, setOrderIdentifier, setQuoteMessage }) {
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', width: '100%' }}>
            <label className="email-label" htmlFor="email">Email (Required)</label>
            <input 
                type="email" 
                id="email" 
                name="email" 
            ...
            />
            <label className="email-label" htmlFor="wallet">Wallet Address (optional)</label>
            <input 
                type="text"
                id="wallet" 
                name="wallet" 
            ...
            <CrossmintPaymentElement
                environment="staging"
                clientId={process.env.REACT_APP_COLLECTIONID}
